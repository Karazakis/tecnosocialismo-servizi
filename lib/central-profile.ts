...
export async function getCentralServiceProfile(headers:Headers):Promise<ServiceProfile|null>{
  ...
  try{const response=await fetch(`${AUTH_ORIGIN}/api/economic-profile`,{headers:{cookie},cache:"no-store"});if(!response.ok)return null;const payload=await response.json() as {profile?:CentralProfile|null};const profile=payload.profile;if(!profile)return null;return{city:profile.city,postalCode:profile.postalCode,radiusKm:profile.radiusKm,needs:profile.basket.filter((item)=>item.enabled&&(item.domain==="services"||item.domain==="education")).map((item)=>({id:item.id,category:portalCategory(item),item:item.item,priority:item.priority})),skills:[...(profile.work.skills??[]),...(profile.contribution.productiveActivities??[])],availability:profile.contribution.availability??"",resources:profile.contribution.resources??[],updatedAt:profile.updatedAt}}catch{return null}
}
export async function getCentralServiceDemand():Promise<ServiceDemand[]>{try{const response=await fetch(`${AUTH_ORIGIN}/api/economic-profile/service-demand`,{cache:"no-store"});if(!response.ok)return[];const payload=await response.json() as {demand?:ServiceDemand[]};return(payload.demand??[]).filter((item)=>item.people>0)}catch{return[]}}
